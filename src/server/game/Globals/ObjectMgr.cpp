...
void ObjectMgr::LoadVehicleSeatAddon()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 vehicle seat addons. DB table `vehicle_seat_addon` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "Table `vehicle_seat_addon`: SeatID: %u does not exist in VehicleSeat.db2. Skipping entry.", seatID);
            continue;
        }
        ...
        if (exitParam >= uint8(VehicleExitParameters::VehicleExitParamMax))
        {
            TC_LOG_ERROR("sql.sql", "Table `vehicle_seat_addon`: SeatID: %u is using invalid exit parameter value (%u). Setting to 0 (none).", seatID, uint32(exitParam));
            continue;
        }
